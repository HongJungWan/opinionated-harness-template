...
| 마커 어노테이션 | `ddd-markers/` | `@AggregateRoot` · `@AggregateInternal` · `@ValueObject` · `@DomainEvent` · `@DomainService` · `@Subdomain` — 훅(이름 기준)과 ArchUnit(클래스 기준)이 같은 마커를 함께 봐요 |
...
수정이 끝난 *완성 파일*과 그 파일이 참조하는 타입(import-follow)까지 읽어서 DDD 규칙으로 검사해요. 규칙은 성격에 따라 두 갈래로 나뉘는데, **기본값은 둘 다 차단(exit 2)** 이에요.

- 🔒 **결정론적 규칙 — 차단(exit 2)** — 입력만으로 정확히 판정돼요. 그 자리에서 고쳐야 해요.
    ...
    - 값 객체 · 이벤트의 가변 노출
- 🔒 **휴리스틱 규칙 — 기본 차단(exit 2), 조정 가능** — 정규식·import-follow 휴리스틱이라 오탐 여지가 있지만 **기본은 차단**이에요. 특정 검사가 너무 깐깐하면 `harness.config.json`의 `checks`에서 `warn`(exit 0, 알려만 줌)·`off`로 낮출 수 있어요.
...
훅과 ArchUnit은 역할이 달라요. 훅은 로컬 에이전트 루프에서 빠르게 도는 1차 강제예요(휴리스틱이라 오탐 여지는 있지만 기본은 차단, 시끄러우면 `checks`로 낮춤). 사람과 CI까지 포함한 권위 있는 최종 게이트는 ArchUnit이 받쳐줘요. 둘의 분담은 [`ARCHUNIT.md`](ARCHUNIT.md)에 정리해 뒀어요.
...
훅(로컬, 빠름, 휴리스틱 허용)과 ArchUnit(CI, 정밀, 오탐 없음)이 2중 방어를 구성해요. 겹치는 구조 규칙은 훅이 로컬에서 즉시 차단하고 ArchUnit이 CI에서 다시 차단하는 **이중 차단**이라, Claude가 그 자리에서 못 고치고 넘어가도 CI가 받쳐줘요. 특히 **도메인 클린코드 5규칙**(Spring 스테레오타입·public setter·비결정 API·도메인서비스 무상태·Typed ID)은 훅(편집 즉시 피드백)뿐 아니라 ArchUnit(CI)에도 두어, **Claude를 거치지 않은 직접 커밋까지** 막아요.
...
| 🔒 훅이 차단 (휴리스틱 · `checks`로 warn/off 조정 가능) | 애그리거트 경계(#9) · ID 참조(#13) · 빈약한 모델(#11) · 최소 애그리거트(#10) · 도메인 서비스(#22) · 팩토리(#15) · BC 격리(#1) · 이벤트 이름(#20) |
| ✅ ArchUnit 정밀 | 레이어 의존·도메인 순수성(#3) · DIP(#4) · 애그리거트 접근(#9·#12) · ID 참조(#13) · 값 객체 불변(#16) · 도메인@Entity 표시 · AR 팩토리 · CORE→GENERIC · Command 명명(#8) · **Spring 스테레오타입 금지 · public setter 금지 · 비결정 API 금지 · 도메인서비스 무상태(#22) · Typed ID(#16) · @Autowired 전면 금지 · AR 컬렉션 비노출(#9) · Command 불변(#8)** (총 18규칙) |
...
- **휴리스틱 규칙도 기본은 차단(block)** 이에요. 정규식이라 오탐 여지가 있지만, 이중 차단의 1차 게이트로서 일단 막고 봐요. 특정 검사가 너무 시끄러우면 `harness.config.json`의 `checks`에서 `warn`(알려만 줌)·`off`로 낮출 수 있어요.
