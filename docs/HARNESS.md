...
훅(로컬, 빠름, 휴리스틱 허용)과 ArchUnit(CI, 정밀, 오탐 없음)이 2중 방어를 구성해요. 겹치는 구조 규칙은 훅에선 경고로 두고 ArchUnit에서 차단해서, 오탐 마찰 없이 커버리지를 넓혀요. 특히 **도메인 클린코드 5규칙**(Spring 스테레오타입·public setter·비결정 API·도메인서비스 무상태·Typed ID)은 훅(편집 즉시 피드백)뿐 아니라 ArchUnit(CI)에도 두어, **Claude를 거치지 않은 직접 커밋까지** 막아요.
